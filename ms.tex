...
To create the raw white light curve, we summed each spectrum over the FIXME pixels in the spectral trace.  The morphology of the white light curve is typical for WFC3 observations \cite{zhou17}: the flux increases increases asymptotically over each orbit (the ``ramp" effect) and there is a visit-long linear trend. The largest ramp occurs in the initial orbit (orbit zero), so we only fit data from orbits one through four in our analysis, following common practice. 

We fit the light curve with the analytic model of the form $F(t) = S(t)\times T(t)$, where $S$ is a systematics model and $T$ is a transit model. We used the same systematics model as FIXME, equation FIXME.  To model the transit, we used the \texttt{batman} package \citep{kreidberg15b}.  In our initial fit, we noticed an increase in flux during orbit three. We also found . We gave the data in orbit three no weight in the fit, and found that the 

In our final fit, we fixed the transit parameters on the values from \cite{dai17}.  


fixed LD on PHOENIX model


\subsection{Spectroscopic Light Curve Fits}
We explored two different methods for fitting the spectroscopic light curves.
