...
We explored several alternate retrieval scenarios, including the addition of cloud patchiness (as described by \citealt{line16}) and a quench pressures for nitrogen and carbon species \citep[to account for transport-induced disequilibrium chemistry, e.g.][]{morley17}.  We did not find significant evidence for either additional complexity in the model, and the main retrieval results were unchanged.

\subsubsection{Methane Abundance}
We ran two additional retrievals to explore whether methane depletion is the underlying cause of the low C/O measurement. In one test, we assumed chemical equilibrium but excluded methane opacity. In this test, the C/O value  

We also performed a ``free" retrieval that allowed the abundances of CH$_4$, H$_2$O and NH$_3$ to vary with no assumption of chemical equilibrium. These results $3\,\sigma$
...
metallicity 30x solar - how does this compare to mass/atmospheric metallicity 
clouds
