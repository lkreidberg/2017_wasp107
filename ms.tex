...
\begin{itemize}
\item{\emph{The upper limit on atmospheric metallicity is $30\times$ solar.} Our estimated upper limit is at the edge of consistency with the Solar System metal enrichment pattern, with predicts a metallicity of $30\times$ solar for WASP-107b \citep{kreidberg14b}.  If the star is old (FIXME Gyr), the upper limit on metallicity would be even lower, implying that WASP-107b has a different formation history from the Solar System planets. Further characterization of the stellar age is needed to explore this scenario.  Either way, this planet stands in contrast to the similar mass and temperature planet GJ 436b, which is thought to have a metallicity greater than $100\times$ solar \citep{morley17}. Together, these exoplanets hint at a greater diversity of compositions than exists in the Solar System.}
...
\item{Optically thick condensates are present at high altitudes ($0.1 - 3$ mbar).  Existing models of physically-motivated cloud and haze formation do not satisfactorily reproduce the observations. The atmospheric scale height is so large (800 km) that clouds form too deep in the atmosphere to loft to the needed height. Hydrocarbon hazes that form at high altitudes can match the data, but these require methane photolysis as a precursor, and methane is depleted in the atmosphere.  Other haze precursors such as $H_2S$ are a possibility and would be worth exploring in future analyses \citep{gao17}.
\citep{crossfield17}} GJ 1214b
\end{itemize}
