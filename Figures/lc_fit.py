...
class LightCurveData:
	...
	def __init__(self, data_file, obs_par, fit_par):
		...
		for i in range(1,len(orb_num)):	
			if (d[i,5] - d[i-1,5]) > 0.2/24.: orb += 1	#stores data as separate visit if gap is longer than 9 hrs; FIXME: what if visits are closer together than 9 hours?
			orb_num[i] = orb	
		...
		#gives third orbit zero weight in fitting (bc there's probably a star spot!)
		#print "deweighting third orbit"
		#ind = orb_num == 2.
		#self.err[ind] = 100000000000.
...
class Model:
	...
	def __init__(self, params, data, flags):
		...
		t_hr = np.linspace(data.time.min()-0.05, data.time.max()+0.05, 1000)
		self.phase_hr = (t_hr - p.t0[0])/p.per[0] - np.floor((t_hr - p.t0[0])/p.per[0])
		self.lc_hr = get_tlc(t_hr, p, data, 0) 
...
def least_sq_fit(file_name, obs_par, fit_par, data, flags):
	...
	ind = model.resid/data.err>3.
	print "Masking outliers:", sum(ind)
	...
	if flags['output']: 
		...
		#pickle.dump([data.wavelength, model.phase, model.data_corr, data.err/data.flux], open("fcorr"+"{0:0.2f}".format(data.wavelength)+".p", "wb"))
		pickle.dump([data, model], open("lsq_output_"+"{0:0.2f}".format(data.wavelength)+".p", "wb"))
		f.close()
